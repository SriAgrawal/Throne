    ...
    if (videoConnection) {
        [[self stillImageOutput] captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:
         ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
             ...
             if([_activeCamera position] == AVCaptureDevicePositionFront){
                 
                 self.stillImage = [UIImage imageWithCGImage:image.CGImage scale:image.scale orientation:UIImageOrientationLeftMirrored];
                 [self setStillImageData:UIImageJPEGRepresentation(self.stillImage, 1.0)];
             }else{
                 [self setStillImage:image];
                 [self setStillImageData:imageData];
             }
             
             if(_previewLayer.frame.size.height != WIN_HEIGHT){
                // CGFloat squareLength = _previewLayer.bounds.size.height;
                 CGSize size = CGSizeMake(_previewLayer.bounds.size.width * 2, _previewLayer.bounds.size.height * 2);
                 
                 UIImage *scaledImage = [self.stillImage resizedImageWithContentMode:UIViewContentModeScaleAspectFill bounds:size interpolationQuality:kCGInterpolationHigh];
                 ...
                 [self setStillImageData:UIImageJPEGRepresentation(croppedImage, 1.0)];
             }
